...
def mouseCallback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        pass  # parking_coordinate
    elif event == cv2.EVENT_RBUTTONDOWN:
        mouse_coordinate.append([x, y])
        if mouse_coordinate:
            mouse_coordinate_all[f"parking_{mouse_coordinate_index}"] = mouse_coordinate
            coordinate_data_handler.update("parking_coordinate", mouse_coordinate_all)
