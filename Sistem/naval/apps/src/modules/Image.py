...
class Vision:
    def __init__(self, isUsingCam=None, addr=None, index=0, device="linux"):
        if device != "linux":  # https://github.com/ultralytics/yolov5/issues/10240
    ...
    @staticmethod
    def imageEncode(frame, format_frame=".jpg"):
        ret, buffer = cv2.imencode(format_frame, frame)
        return buffer
    ...
    @staticmethod
    def writeImage(frame, path="cats-output.png", debug=False):
        filename = path
    ...
    @staticmethod
    def blur(frame=None, sigma=11):
        return cv2.GaussianBlur(frame, (sigma, sigma), 0)

    @staticmethod
    def autoContrast(frame):
    ...
    @staticmethod
    def adaptiveContrast(frame, clip_limit=2.0, tile_grid_size=(8, 8)):
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    ...
    @staticmethod
    def enhanceColors(frame, saturation_factor):
    ...
    @staticmethod
    def setBrightnessNcontrast(frame, bright=0.0, contr=0.0, beta=0.0):
        return cv2.addWeighted(frame, 1 + float(contr)
                               / 100.0, frame, beta, float(bright))

    def read(self, frame_size=0, show_fps=False, loop=False):
        try:
            success = False
            frame = []
            while not success:
                ...
                if not success:
                    if not self.is_using_cam and loop:
                        self.cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                    else:
                        raise RuntimeError("Failed to capture frame !!")
            if show_fps:
                try:  # put fps
                    cv2.putText(frame, str(self.getFps()) + " fps", (20, 40), 0, 1,
