...
class Drawing:
    ...
    #     pass
    def setPolylinesArea(self, frame, area):
        cv2.polylines(frame, [np.array(area, np.int32)], True, (0, 255, 0), 2)
        return np.array(area, np.int32)

    def isInPolylinesArea(self, area, coordinates):
        result = cv2.pointPolygonTest(area, (coordinates[0], coordinates[1]), False)
        if result >= 0:
            return 1
        return 0
